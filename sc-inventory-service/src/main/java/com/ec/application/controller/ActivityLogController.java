...
import com.ec.application.service.ActivityLogGlobalSyncService;
...
public class ActivityLogController {
    ...
    @Autowired
    private ActivityLogGlobalSyncService activityLogGlobalSyncService;

    @Autowired
    private ActivityLogGlobalSyncOrchestrator activityLogGlobalSyncOrchestrator;
    ...
    @ResponseStatus(HttpStatus.OK)
    public Page<GlobalActivityLog> getGlobalActivityLog(
            @RequestBody FilterDataList filterDataList,
            @PageableDefault(page = 0, size = 50, sort = "activityTime", direction = Sort.Direction.DESC) Pageable pageable)
            throws Exception {
        return activityLogGlobalSyncService.getFiltered(filterDataList, pageable);
    }

    @UseDefaultTenant
    @PostMapping("/global/export")
    public ResponseEntity<byte[]> exportGlobalActivityLog(@RequestBody FilterDataList filterDataList) throws Exception {
        byte[] excel = activityLogGlobalSyncService.exportExcel(filterDataList);
        return ResponseEntity.ok()
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"Global_Activity_Log.xlsx\"")
                .contentType(MediaType.parseMediaType(
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
                .body(excel);
    }
    ...
    public String triggerGlobalSync() {
        return activityLogGlobalSyncOrchestrator.syncAllTenants();
    }
}
