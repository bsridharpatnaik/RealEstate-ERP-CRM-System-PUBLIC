...
public class StockSpecification {
    ...
    public static Specification<Stock> getSpecification(FilterDataList filterDataList) {
        ...
        if (productCodes != null && productCodes.size() > 0)
            finalSpec = specbldr.specAndCondition(finalSpec,
                    specbldr.whereChildFieldContains(Stock_.PRODUCT, Product_.PRODUCT_CODE, productCodes));
        ...
        if (globalSearch != null && globalSearch.size() > 0) {
            ...
            internalSpec = specbldr.specOrCondition(internalSpec,
                    specbldr.whereChildFieldContains(Stock_.PRODUCT, Product_.PRODUCT_CODE, globalSearch));
            internalSpec = specbldr.specOrCondition(internalSpec,
