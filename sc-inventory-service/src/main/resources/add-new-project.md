...
**Do not use `SELECT *`** for ANY of these tables. Hibernate creates the new
schema's columns in alphabetical order while `masterschema` uses creation order,
so `SELECT *` maps by position and corrupts data (e.g. `Product` fails with
`Incorrect integer value: 'NOS' for column 'default_expiry_days'`; `contacts`
puts `createdBy` into `creationDate`).

Use the dynamic column-list insert below — the column list is fetched from
`INFORMATION_SCHEMA` at runtime, so it always matches and stays current when
columns are added. Set `@target` once, then run the block for each table.

```sql
SET @target = 'dextension';  -- change this for each new project

-- Category
SET @cols = (SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY ORDINAL_POSITION)
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA='masterschema' AND TABLE_NAME='Category');
SET @sql = CONCAT('INSERT INTO ',@target,'.Category (',@cols,') SELECT ',@cols,' FROM masterschema.Category');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Product
SET @cols = (SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY ORDINAL_POSITION)
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA='masterschema' AND TABLE_NAME='Product');
SET @sql = CONCAT('INSERT INTO ',@target,'.Product (',@cols,') SELECT ',@cols,' FROM masterschema.Product');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Machinery
SET @cols = (SELECT GROUP_CONCAT(COLUMN_NAME ORDER BY ORDINAL_POSITION)
  FROM INFORMATION_SCHEMA.COLUMNS
  WHERE TABLE_SCHEMA='masterschema' AND TABLE_NAME='Machinery');
SET @sql = CONCAT('INSERT INTO ',@target,'.Machinery (',@cols,') SELECT ',@cols,' FROM masterschema.Machinery');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
...
SET @sql = CONCAT('INSERT INTO ',@target,'.contacts (',@cols,') SELECT ',@cols,' FROM masterschema.contacts');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
```

Only change `@target` for each new project. Column lists are always current — no
manual updates needed when columns are added to any of these tables.
...
| 4 | Seed Category / Product / Machinery / contacts | dynamic column-list INSERT from `masterschema` (never `SELECT *`) |
