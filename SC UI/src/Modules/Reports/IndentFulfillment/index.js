...
// Status groups for quick-filter chips
const STATUS_GROUPS = [
  ...
  { key: 'IN_PROGRESS',      label: 'In Progress',  color: '#3498db', bg: '#ebf5fb', statuses: ['PO CREATED', 'PO PARTIAL', 'INWARD PARTIAL'] },
  { key: 'COMPLETED',        label: 'Completed',    color: '#27ae60', bg: '#eafaf1', statuses: ['CLOSED', 'PO COMPLETED'] },
...
class IndentFulfillmentList extends ListCommon {
  ...
  handleGroupClick(group) {
    ...
    if (group.key === 'ALL' || isActive) {
      ...
      // Set first status in group as filter (backend supports single value)
      // For multi-status groups, we pass the first one — user can refine via filter panel
      this.filterData.indentStatus = group.statuses ? group.statuses[0] : '';
      ...
      <div className="page">
        ...
        <div className="list-section">
          ...
          <div style={{ fontSize: 12, color: '#888', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
            <span>ℹ️</span>
            <span>
              Shows indent line items with requested, received, and pending quantities.
              Red <strong>Pending</strong> qty = action required. Overdue need-by dates shown in red.
            </span>
          </div>
            ...
            <div>
              {this.state.totalRecords > 0 && (
                <span style={{ fontSize: 13, color: '#555' }}>
                  ...
                  {this.state.activeGroup !== 'ALL'
                    ? ` · ${STATUS_GROUPS.find(g => g.key === this.state.activeGroup)?.label}`
                    : ''}
                </span>
              )}
            </div>
