...
public interface LeadActivityRepo extends BaseRepository<LeadActivity, Long>, JpaSpecificationExecutor<LeadActivity>
{
	...
	@Query(value = "SELECT l from LeadActivity l where l.lead.leadId=:leadId and l.isOpen=true and l.activityType='Payment' order by l.activityDateTime")
	List<LeadActivity> fetchPendingPaymentActivitiesForLead(@Param("leadId") Long leadId);
	...
	@Query(value = "SELECT count(distinct la) from LeadActivity la where la.activityType='Property_Visit' and la.creatorId=:id and la.created>=:dateStart and la.isRescheduled=false")
	Long getpropertyvisit(@Param("id") Long id, @Param("dateStart") Date dateStart);
	...
	@Query(value = "SELECT la.leadActivityId from LeadActivity la where TIMESTAMPDIFF(MINUTE,now(),activityDateTime) < 10 and TIMESTAMPDIFF(MINUTE,now(),activityDateTime) > 0")
	List<Long> findUpcomingActivities();

	@Query(value = "SELECT la.lead.leadId from LeadActivity la where la.leadActivityId = :leadActivityId")
